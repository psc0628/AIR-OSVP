...
class ViewStateTransformer(nn.Module):
    def __init__(self, config):
        super(ViewStateTransformer, self).__init__()

        input_size = config.vs_config.input_size
        hidden_size = config.vs_config.hidden_size
        num_heads = config.vs_config.num_heads
        num_layers = config.vs_config.num_layers
        dropout = config.vs_config.dropout
    ...
    def forward(self, input):
        ...
        embedded = self.embedding(input)  # embedded shape: (batch_size, hidden_size)
        embedded = embedded.unsqueeze(0)  # embedded shape: (1, batch_size, hidden_size)
        transformer_output = self.transformer(embedded)  # transformer_output shape: (1, batch_size, hidden_size)
        features = transformer_output.squeeze(0)  # features shape: (batch_size, hidden_size)
        features = self.fc(features)  # features shape: (batch_size, hidden_size)
        features = self.dropout(features)  # features shape: (batch_size, hidden_size)

        features = self.increase_dim(features)
        return features
...
class COSC(nn.Module):
    def __init__(self, config, **kwargs):
        super().__init__()
        self.num_query = config.num_query   # 512
        self.num_points = getattr(config, 'num_points', None)   # 8192

        self.base_model = PCTransformer(config)
        self.vs_model = ViewStateTransformer(config)

        self.predict = nn.Sequential(
            nn.Linear(config.global_feature_dim , 1024),
            nn.GELU(),
            nn.Linear(1024, 1024),
            nn.GELU(),
            nn.Linear(1024, config.output_dim)
    ...
    def forward(self, xyz, vs):
        # xyz = fps(xyz, self.num_points)
        pcs_feature = self.base_model(xyz) # B M C and B M 3
        vs_feature = self.vs_model(vs)

        features = pcs_feature + vs_feature
        pred = self.predict(features)

        return self.sig(pred)