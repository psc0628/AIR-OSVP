...
def main(config, args):
    ...
    scene_file_path = os.path.join(cosc_root_dir, args.obj_name + "_scene.txt")
    ...
    epoch = epoch_start
    max_epoch = epoch_start + args.epoch
    best_train_iou = 0.0
    ...
    while epoch < max_epoch:
        ...
        if best_train_iou < train_iou:
            ...
            #     {
            #         "epoch": epoch,
            #         "state_dict": best_net_state_dict,
            #         "optimizer": optimizer.state_dict(),
            #     },
            #     "./new_checkpoint.pth"
            # )

        # logs_file(os.path.join(savedir_root, args.obj_name + "_logs.csv"), epoch, train_log_data)
    ...
    # infer the model
    bbx_dir = "./data/"
    infer_bbx_path = os.path.join(bbx_dir, args.obj_name + "_bbx.txt")
    inp_pts = np.loadtxt(infer_bbx_path)
    ...
    #for data in train_loader:
        ...
        if config['predict_pc']:
            print("predicting pointclouds")
            pred_pcs = np.array([[0., 0., 0.]], dtype=np.float32)

            while inp_pts.shape[0] > 0:
                idx = np.random.choice(inp_pts.shape[0], min(32768, inp_pts.shape[0]), replace=False)
                sub_pcs = inp_pts[idx]
                sub_labels = export_pointcloud_region_growing_v2(net, latent, input_points=sub_pcs,
                                                                 device=device).squeeze(0)

                pred_sub_pcs = sub_pcs[sub_labels == 1]
                # print(f"pred_sub_pcs.shape: {pred_sub_pcs.shape}")
                pred_pcs = np.concatenate((pred_pcs, pred_sub_pcs), axis=0)

                inp_pts = np.delete(inp_pts, idx, axis=0)
                # print(f"inp_pts.shape: {inp_pts.shape}")

                save_path = os.path.join(savedir_root, args.obj_name + "_pc.txt")
                # print(f"saving pointcloud to {save_path}")

            # print(f"saving pointcloud shape: {pred_pcs.shape}")
            pred_pcs = pred_pcs[1:]

            np.savetxt(save_path, pred_pcs, fmt='%.4f', delimiter=' ')
        else:
            print("predicting mesh")
            mesh = export_mesh_and_refine_vertices_region_growing_v2(net, latent, device=device,
                                                                     input_points=inp_pts, padding=1, step=0.002,
                                                                     out_value=1)
            save_path = os.path.join(savedir_root, f"out_view_{scene_idx}.ply")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--config_default', type=str, default="./POCO/configs/config_default.yaml")
    ...
    np.savetxt("./run_time/" + args.obj_name + "_time_poco.txt", np.asarray([endTime - startTime]), fmt='%.6f')

# python train_and_infer_one.py --config configs/config_cosc.yaml --obj_name Dragon --epoch 50
