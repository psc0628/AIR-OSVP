...
//Combined_on 0 -> NBV
//Combined_on 1 -> NBV + (MA-)SCVP
//Combined_on 2 -> NBV + IS-EVP
...
public:
	...
	Share_Data(string _config_file_path, string test_name = "", int test_rotate = -1, int test_view = -1, int test_method = -1, int move_test_on = -1, int combined_test_on = -1)
	{
		...
		fs["evpis_net_path"] >> evpis_net_path;
		...
		if (method_of_IG == 11 || method_of_IG == 7 || Combined_on > 0 ) num_of_max_iteration = 32; //SCVP系列不限制最大值
		if (Combined_on <= 0) num_of_nbvs_combined = 0; //非综合系统不设置初始NBV个数偏移(用于指示MASCVP)
		/* //最大重建数量为默认20
		if (method_of_IG != 7) {
		...
		*/
		...
		if (fin_GT_points_number.is_open()) {
			...
			cout << "no GT_points_number, run mode 1 first. This process will continue without GT_points_number." << endl;
		}
		...
		if (Combined_on > 0 ) save_path += "_combined_" + to_string(num_of_nbvs_combined);
...
/*
//bbx_2mm 搜索距离足够近
cout << "share_data->predicted_size: " << share_data->predicted_size << endl;
ofstream fout_bbx_2mm(output_path + "/" + to_string(iteration) + "/full_bbx.txt");
for (double x = share_data->object_center_world(0) - share_data->predicted_size; x < share_data->object_center_world(0) + share_data->predicted_size; x += share_data->ground_truth_resolution)
	for (double y = share_data->object_center_world(1) - share_data->predicted_size; y < share_data->object_center_world(1) + share_data->predicted_size; y += share_data->ground_truth_resolution)
		for (double z = max(share_data->min_z_table, share_data->object_center_world(2) - share_data->predicted_size); z < max(share_data->min_z_table, share_data->object_center_world(2) - share_data->predicted_size) + 2.0 * share_data->predicted_size; z += share_data->ground_truth_resolution) {
			...
			//搜索到了距离足够近
			//if (sqrt(pointKNNSquaredDistance[0]) > 6.0 * share_data->ground_truth_resolution) {
			if (sqrt(pointKNNSquaredDistance[0]) > share_data->predicted_size) {
				continue;
			}
